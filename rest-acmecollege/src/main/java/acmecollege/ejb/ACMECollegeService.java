...
//        so this would be something like SecurityRole.findRoleByName(USER_ROLE)
//        and the query would be like "SELECT r FROM SecurityRole r where r.name = :param1" ??
    ...
    public void deleteStudentById(int id) {
        ...
        if (student != null) {
            em.refresh(student); 
            TypedQuery<SecurityUser> findUser = em.createNamedQuery(SecurityUser.USER_FOR_OWNING_STUDENT_QUERY, SecurityUser.class)
	...
	public void deleteClubMembershipById(int id) {
		ClubMembership cm = getClubMembershipById(id);
		if (cm != null) {
			em.remove(cm);
		}
	}
    
    public List<ClubMembership> getAllRecords() {
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<ClubMembership> cq = cb.createQuery(ClubMembership.class);
		Root<ClubMembership> root = cq.from(ClubMembership.class);
		cq.select(root);
		TypedQuery<ClubMembership> tq = em.createQuery(cq);
		List<ClubMembership> reslist = tq.getResultList();
		return reslist;
	}
